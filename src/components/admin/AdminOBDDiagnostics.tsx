/**
 * Admin OBD Diagnostics — Full diagnostic center with customer/vehicle selector
 * Admin gets ALL OBD features: live data, DTC read/clear, CAN analysis, UDS, coding, etc.
 */

import { useState, useCallback, useMemo } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  Gauge, Bluetooth, Terminal, Settings, Database, Radar, Activity,
  Brain, ScrollText, Cpu, Code, Car, TrendingUp, Shield, Layers,
  Search, Settings2, Wrench, ChevronUp, Users, Scan
} from "lucide-react";
...
import { VehicleSelector } from "@/components/obd/VehicleSelector";
      ...
      <Card>
        <CardContent className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Select value={selectedUserId || ''} onValueChange={(v) => { setSelectedUserId(v); setSelectedVehicleId(null); }}>
              <SelectTrigger>
            ...
            <Select value={selectedVehicleId || ''} onValueChange={setSelectedVehicleId} disabled={!selectedUserId}>
              <SelectTrigger>
          ...
          {selectedProfile && (
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <Badge variant="secondary">{selectedProfile.full_name || 'Zákazník'}</Badge>
              {selectedVehicle && (
      ...
      <div className="min-h-[400px]">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {activeTab === 'dashboard' && (
              <InteractiveDashboard data={liveData} active={elmReady} />
            )}
            ...
            {activeTab === 'uds' && <Suspense fallback={<Fallback />}><UDSView elmReady={elmReady} /></Suspense>}
            {activeTab === 'discover' && <Suspense fallback={<Fallback />}><DiscoveryView elmReady={elmReady} /></Suspense>}
            {activeTab === 'decoder' && <Suspense fallback={<Fallback />}><SensorDecoderView elmReady={elmReady} /></Suspense>}
            {activeTab === 'logging' && <Suspense fallback={<Fallback />}><LoggingView elmReady={elmReady} /></Suspense>}
            {activeTab === 'reverse' && <Suspense fallback={<Fallback />}><ReverseEngineeringView elmReady={elmReady} /></Suspense>}
            {activeTab === 'orchestrator' && <Suspense fallback={<Fallback />}><OrchestrationPanel /></Suspense>}
            {activeTab === 'security' && <Suspense fallback={<Fallback />}><SecurityFlashView /></Suspense>}
            {activeTab === 'trends' && <Suspense fallback={<Fallback />}><TrendChartsView elmReady={elmReady} /></Suspense>}
            {activeTab === 'vehicle3d' && <Suspense fallback={<Fallback />}><Vehicle3DView elmReady={elmReady} /></Suspense>}
            {activeTab === 'devmode' && <Suspense fallback={<Fallback />}><DevModeView onSend={sendCommand} elmReady={elmReady} /></Suspense>}
            {activeTab === 'settings' && <Suspense fallback={<Fallback />}><SettingsView commandDelay={commandDelay} onDelayChange={setCommandDelay} /></Suspense>}
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
