 ...
 * ✅ OPRAVENO: používá globalOemSearch() + final_price
 * Hybrid lookup: local OEM + J+M API
 */
...
const GlobalOEMSearch = forwardRef<HTMLDivElement, Props>(
  ({ onOrder }, ref) => {
    ...
    const search = async () => {
      ...
      try {
        ...
        console.error("[GlobalOEMSearch] search failed:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <div
        ref={ref}
        className="rounded-xl border border-border/40 bg-card/50 backdrop-blur-sm p-3 mb-6"
      >
        {/* search bar */}
...
const ResultRow = ({
  ...
  const displayPrice = p.final_price ?? p.price_with_vat;
    ...
    >
      ...
      <div className="text-right shrink-0">
        <div
          className={cn(
            "text-sm font-bold",
            isOem ? "text-primary" : "text-foreground"
          )}
        >
          {formatPrice(displayPrice)}
        </div>

        {!isOem && p.markup_percent > 0 && (
          <div className="text-[10px] text-muted-foreground">
            +{p.markup_percent}% marže
          </div>
        )}
      </div>
