...
const Catalog = forwardRef<HTMLDivElement>((_, ref) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(
          `[Catalog] Parallel fetch: OEM(local) + J+M(vehicle) for ${brand} ${model} ${engine}`
        );
        ...
        if (allJm.length === 0 && oemItems.length === 0 && page === 0) {
          setJmWarning(
            jmVehiclePayload.warning ||
              `Pro ${brand} ${model}${
                engine ? " · " + engine : ""
              } se nepodařilo načíst aftermarket díly z J+M.`
          );
        } else {
          setJmWarning(null);
        }
    ...
    try {
      ...
      const unitPrice =
        ...
        (p.price_with_vat !== null
          ? Math.round((p.price_with_vat / 1.21) * 100) / 100
          : null);
  ...
  const goBack = () => {
    ...
    else if (step === "category" && categoryPath.length > 0)
                ...
                <div className="flex items-center gap-1.5 mt-1 text-xs text-muted-foreground flex-wrap">
                  {breadcrumb.map((b, i) => (
                    <span key={i} className="flex items-center gap-1.5">
                      ...
                      <span
                        className={
                          i === breadcrumb.length - 1 ? "text-primary font-medium" : ""
                        }
                      >
                        {b}
                      </span>
                ...
                {visibleCategories.map((c) => {
                  ...
                  return (
                    <button
                      key={c.id}
                      onClick={() => {
                        if (hasChildren && !categoryQuery)
                          setCategoryPath((path) => [...path, c.id]);
                        else setCategory(c);
                      }}
                      className="group w-full flex items-center justify-between gap-3 px-4 py-3 hover:bg-secondary/50 transition-colors text-left"
                    >
            ...
            {!listLoading && !jmLoading && items.length === 0 && (
              <div className="mt-6 p-6 rounded-2xl border border-dashed border-border/60 bg-card/40 text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Nenašli jsme specifické díly pro tento motor, zkuste hledat podle OEM
                  kódu.
                </p>
