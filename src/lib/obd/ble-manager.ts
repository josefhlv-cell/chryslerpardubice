...
private async performConnect(deviceId: string): Promise {
try {
await this.ensureInitialized();
...
private async probeProfile(deviceId: string, profile: OBDProfile): Promise {
...
private async initializeELM327(): Promise {
...
async readResponse(timeout = 2000): Promise {
if (!this.isNative) {
