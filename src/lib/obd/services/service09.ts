...
function sanitizeVin(chars: number[]): string {
  ...
  let vin = s.replace(/[^A-HJ-NPR-Z0-9]/gi, "").toUpperCase();
  // Některé ELM klony při indexed/multi-line odpovědi zdvojí Mode09 prefix
  // 49 02 01. Po sanitizaci by z toho vzniklo "490201<vin>".
  if (vin.startsWith("490201")) vin = vin.slice(6);
  return vin;
}

async function tryMode09(): Promise<VinReadResult | null> {
  const res = await elmQueue.send("0902", { timeoutMs: 3800, commandType: "read_vin" });
...
async function tryUdsF190AtHeader(header: string): Promise<VinReadResult | null> {
  ...
  const res = await elmQueue.send("22F190", { timeoutMs: 3800, commandType: "read_vin" });
...
async function tryUdsF190(): Promise<VinReadResult | null> {
  ...
  try {
    ...
    try { await elmQueue.send("ATFCSD300000", { timeoutMs: 700 }); } catch { /* ignore */ }
    try { await elmQueue.send("ATFCSM1", { timeoutMs: 700 }); } catch { /* ignore */ }
  }
}
...
export async function readVinMode09(): Promise<VinReadResult> {
  return elmQueue.runExclusive(async () => {
    ...
    await elmQueue.send("ATSTC8", { timeoutMs: 800 }).catch(() => undefined);
    try {
      const primary = await tryMode09();
      if (primary) return primary;
      const fallback = await tryUdsF190();
      if (fallback) return fallback;
      const res = await elmQueue.send("0902", { timeoutMs: 2600, commandType: "read_vin" });
      const cleaned = cleanElmResponse(res.raw, "0902");
      return {
