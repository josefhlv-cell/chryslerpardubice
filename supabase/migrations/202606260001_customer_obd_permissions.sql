...
CREATE POLICY "Admins manage customer obd permissions"
ON public.customer_obd_permissions
FOR ALL
TO authenticated
USING (has_role(auth.uid(), 'admin'))
WITH CHECK (has_role(auth.uid(), 'admin'));

CREATE POLICY "Customers read own obd permissions"
ON public.customer_obd_permissions
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY "Admins read obd audit"
ON public.customer_obd_permission_audit
FOR SELECT
TO authenticated
USING (has_role(auth.uid(), 'admin'));

CREATE POLICY "Admins insert obd audit"
ON public.customer_obd_permission_audit
FOR INSERT
TO authenticated
WITH CHECK (has_role(auth.uid(), 'admin'));