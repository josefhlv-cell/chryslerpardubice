...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!FC_KEY) {
      return j({ success: false, error: "FIRECRAWL_API_KEY not configured" }, 500);
    }
    ...
    if (!role) {
      return j({ success: false, error: "Forbidden" }, 403);
    }

    const { brand, model, year, engine, max_pages = 50 } = await req.json();

    if (!brand || !model) {
      return j({ success: false, error: "brand and model required" }, 400);
    }
    ...
    if (!crawlResp.ok) {
      ...
      return j(
        {
          success: false,
          error: `Firecrawl: ${crawlResp.status} ${errText.slice(0, 200)}`,
        },
        502,
      );
    }
    ...
    if (!jobId) {
      return j({ success: false, error: "Firecrawl no job id" }, 502);
    }
    ...
    for (let i = 0; i < 30; i++) {
      ...
      if (sd.status === "failed") {
        return j({ success: false, error: "Firecrawl crawl failed" }, 502);
      }
    }

    const oemRegex =
      /\b([0-9]{4,8}[A-Z]{1,3}[0-9A-Z]{0,4}|[K6][0-9]{6,9})\b/g;
    ...
    if (insErr) {
      return j({ success: false, error: insErr.message }, 500);
    }
...
// ✅ FIX: chybějící response helper (hlavní bug)
